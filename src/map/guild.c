...
void do_init_guild(bool minimal) {
	...
	sv->readdb(map->db_path, DBPATH"castle_db.txt", ',', 4, 5, -1, guild->read_castledb);

	sv->readdb(map->db_path, DBPATH"guild_skill_tree.txt", ',', 2+MAX_GUILD_SKILL_REQUIRE*2, 2+MAX_GUILD_SKILL_REQUIRE*2, -1, guild->read_guildskill_tree_db); //guild skill tree [Komurka]
