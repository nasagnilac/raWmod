					...
					while (i >= 0) {
						...
						if (d_sc->data[type2]) {
							status->change_start(bl, bl, type2, 10000, d_sc->data[type2]->val1, 0, 0, 0,
							                     ...
							                     ((type2 != SC_DEFENDER) && (type2 != SC_AUTOGUARD) && (type2 != SC_REFLECTSHIELD) ) ? SCFLAG_NOICON : SCFLAG_NONE);
						}
						i--;
					}
				}
...
int status_readdb(void)
{
	...
	// read databases
	//
	sv->readdb(map->db_path, DBPATH"job_db2.txt",         ',', 1,                 1+MAX_LEVEL,       -1,                       status->readdb_job2);
	...
	sv->readdb(map->db_path, DBPATH"sc_config.txt",       ',', 2,                 2,                 SC_MAX,                   status->readdb_scconfig);
	status->read_job_db();
