...
void mob_load(bool minimal) {
	...
	sv->readdb(map->db_path, DBPATH"mob_item_ratio.txt", ',', 2, 2+MAX_ITEMRATIO_MOBS, -1, mob->readdb_itemratio); // must be read before mobdb
	mob->readchatdb();
	mob->readdb();
	mob->readskilldb();
	sv->readdb(map->db_path, DBPATH"mob_avail.txt", ',', 2, 12, -1, mob->readdb_mobavail);
